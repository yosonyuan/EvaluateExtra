...
import com.tencent.map.geolocation.iodetector.provider.IoGpsWifiManager;

public class LocationTestActivity extends ActionBarActivity implements
		Observer, OnClickListener,IOListener {
	...
	// 室内外检测
	private IODetectorManager mIoDetectorManager;
	private IoGpsWifiManager mGpsWifiManager;
...
//				}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mTvEnv = (TextView) findViewById(R.id.tv_enviroment);
		...
		mIoDetectorManager = IODetectorManager.getInstance(getApplicationContext());
		mGpsWifiManager = IoGpsWifiManager.getInstance(getApplicationContext());
	}
...
//			mTvEnv.setTextColor(Color.RED);
//		} else {
//			mTvEnv.setTextColor(Color.BLACK);
//		}
	...
	void runOrStop(MenuItem item) {
		if (mBound) {
			...
			if (start) {
				...
				mIoDetectorManager.removeIOListener(this);
				mIoDetectorManager.stop();
				mGpsWifiManager.stop();
				mTvEnv.setText("--");
			} else {
				...
				mIoDetectorManager.addIOListener(this);
				mIoDetectorManager.start(getMainLooper());
				mGpsWifiManager.start(getMainLooper());
			}
...
//	}
	...
	@Override
	public void onIOEnvListener(int env) {
		mTvEnv.setText(IODetectorManager.getEnvironmentDesc(env));
	}
}
